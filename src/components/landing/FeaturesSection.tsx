  ...
  return <section className="py-24 px-6" ref={ref}>
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate={isInView ? 'visible' : 'hidden'} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {features.map((feature, index) => <motion.div key={feature.title} variants={itemVariants} className={cn('group relative p-8 rounded-2xl border border-border bg-card overflow-hidden transition-all duration-300 hover:shadow-echo-lg hover:border-border/80')}>
              {/* Background gradient */}
              ...
              {/* Hover line */}
              <motion.div className="absolute bottom-0 left-0 h-1 bg-foreground/10" initial={{
            width: 0
          }} whileHover={{
            width: '100%'
          }} transition={{
