        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="relative">
              <div className="flex items-end gap-4">
                {/* Tablet */}
                <motion.div
                  className="w-48 h-64 rounded-2xl bg-card border-4 border-foreground/10 shadow-echo-lg overflow-hidden"
                  animate={
                ...
                {/* Phone */}
                <motion.div
                  className="w-32 h-56 rounded-[2rem] bg-card border-4 border-foreground/10 shadow-echo-lg overflow-hidden"
                  animate={
                    !shouldReduceMotion && isInView
                      ? { y: [0, -8, 0] }
                      : { y: 0 }
              ...
              <motion.div
                className="absolute -bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 px-4 py-2 bg-card rounded-full shadow-echo border border-border"
                animate={
                  !shouldReduceMotion && isInView
                    ? { scale: [1, 1.05, 1] }
                    : { scale: 1 }
          ...
          >
            ...
            <p className="text-lg text-muted-foreground mb-6">
              Echo works seamlessly across all your devices. Install it like a
              native app, use it offline, and pick up exactly where you left off.
            </p>

            <ul className="space-y-3 mb-8">
              {[
                'Instant loading, even on slow connections',
                'Offline-first: your session is always available',
                'Syncs automatically when you are back online',
                'No app store required — install from your browser',
              ].map((item, index) => (
                <motion.li
                  key={item}
